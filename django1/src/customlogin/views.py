        ...
        if f.is_valid():
            ...
            #비밀번호 다른경우
                ...
                return render(request, 'cl/signup.html',
                              {'f':f, 'error':'비밀번호가 같지않습니다.'})
        #유효한 값이 아닌경우
        else:
            #HTML파일에 에러코드 전달
            #에러코드가 is_valid()함수를 통과하지 못하면 User모델클래스에 설정된
            #에러정보가 자동으로 넘겨짐
            return render(request, 'cl/signup.html',
                          {'f':f})
...
#2)받은 결과가 None값이 아닌경우, login함수를 통해 클라이언트를 로그인함
...
def signin(request):
    ...
    #POST
        ...
        if u:
            #로그인 처리
            login(request, u)
            #nexturl : 이동할 URL주소가 있는경우 메인페이지로 가지않고 저장된 URL로
            #리다이랙트 
