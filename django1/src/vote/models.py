...
class Choice(models.Model):
    #ForeignKey(연결할 다른모델클래스) : ForeignKey 객체를 만든 모델클래스의 객체들이
    #연결한 모델클래스의 객체와 1:n 관계로 연결할 수 있는 설정
    #ForeignKey의 객체를 저장한 변수는 연결한 모델클래스의 객체를 저장하는 변수가 됨
    #Choice객체.q.name -> 해당하는 Choice객체와 연결된 Question객체의 name변수값을
    #                    추출
    #on_delete : 연결된 모델클래스의 객체가 삭제될때 어떻게 처리할지 지정하는 변수
    #on_delete = models.PROTECT : 연결된 모델클래스의 객체가 삭제되지않도록 막아주는 기능
