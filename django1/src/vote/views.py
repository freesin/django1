...
def index(request):
    ...
    #ex) a = [1,2,3,4,5]
    #a[1:4] => [2,3,4]
    #시작인덱스가 빈칸인 경우 -> 0번인덱스부터 추출
    #종료인덱스가 빈칸인 경우 -> 마지막인덱스의 요소까지 추출
...
def detail(request, q_id):
    ...
    #모델클래스A의객체.모델클래스B_set : A모델클래스와 B모델클래스가 1:n관계인 경우
    #해당 A객체와 연결된 B객체들을 대상으로 get(),all(),filter(),exclude()함수들을
    #사용할 수 있음
...
def vote(request):
    ...
    #=> HTML의 form태그의 요청방식을 POST로 했기때문에
    #request.method : 사용자의 요청방식이 문자열형태로 저장된 변수
    #                 대소문자 구분이 있으므로 비교연산자 사용시 "GET", "POST"로 비교
    if request.method == "POST":
        #request.POST : 클라이언트가 POST 방식으로 요청할때 넘어온 데이터가 저장된변수
        #request.GET : 클라이언트가 GET 방식으로 요청할때 넘어온 데이터가 저장된변수
...
#데코레이터 : URLConf를 통해 View함수가 호출 될 때, 뷰가 실행되기 전
#          먼저 실행되는 함수.
...
#login_required : 뷰함수 호출 전 요청을 한 클라이언트가 비로그인 상태
#인경우, 웹프로젝트에 지정된 로그인 URL로 넘어가는 데코레이터 함수
...
#'질문추가' 링크를 타고온 클라이언트에게는 비어있는 QuestionForm 기반의 입력양식을 제공
#폼을 제출한 경우, 클라이언트가 작성한 정보를 바탕으로 Question객체를 생성 및 DB에 저장
#-> 완성된 객체에 대한 detail 뷰 호출
@login_required
def qregister(request):
    #사용자의 요청방식을 구분해 처리
    #사용자 요청이 GET방식일 때의 처리
    if request.method == "GET":
        #QuestionForm 객체 생성
        #모델폼클래스 객체 생성 시 매개변수에 아무런 값도 전달하지 않는 경우, 입력양식에
        #값이 비어있는 형태로 객체가 생성됨
        f = QuestionForm()

        #HTML 파일에 폼객체 전달
        return render(request, 'vote/qregister.html',
                      {'f' : f})
    #사용자 요청이 POST방식일 때의 처리
        ...
        if f.is_valid():
            #폼객체.is_valid() : 사용자 입력이 유효한값인경우 True,
            #유효하지 않는 값이 입력되면 False 반환
            #폼객체.is_valid() 결과로 True가 반환된 경우, 폼객체에 들어있는 각
            ...
            #QuestionForm으로 Question 객체 저장 시, date변수에 값이 없어 
            #데이터베이스에 저장 시 에러가 발생
            #모델폼객체.save(commit=False) : 연동된 모델클래스의 객체로 변환 및 반환
            q = f.save(commit=False) #Question객체 생성 후 q변수에 저장
...
def qupdate(request, q_id):
    ...
    if request.method == 'GET':
        ...
        #qregister.html과 유사하게 HTML코드가 구현되기 때문에 기존에 만들어진
        #HTML파일을 재사용. form태그의 action속성을 빈칸으로 처리했기 때문에,
        #qregister 뷰에서 qregister.html를 쓴 것과, qupdate 뷰에서 qregister.html를
        #쓴 결과가 각자의 뷰함수로 POST요청을 함
...
def qdelete(request, q_id):
    ...
    #해당 객체를 데이터베이스에서 삭제함. 삭제한 객체에 저장된 변수값은 
    #사용할 수 있음
    q.delete()
...
def cregister(request):
    ...
    if request.method == 'GET':
        ...
        if f.is_valid(): #사용자 입력이 유효한 값일때의 처리
            #사용자 입력으로 Choice 객체에 변수들의 값이 채워진 상태이므로,
            #바로 데이터베이스에 저장해도 됨
...
def cupdate(request, c_id):
    ...
    #POST
        ...
        if f.is_valid():
            #데이터베이스에 수정사항 반영
            #c변수와 f.save()함수에서 주는 Choice객체가 동일하기때문에 변수에 저장하지않음
            f.save() 
            #다른 URL을 사용자에게 전송 
            return HttpResponseRedirect(reverse('vote:detail',
                                                args=(c.q.id,) ) )
...
def cdelete(request, c_id):
    c = get_object_or_404(Choice, id=c_id)
    c.delete() #데이터베이스에서 Choice객체 삭제, 뷰함수 내에서는 변수값을 사용할 수 있음
    return HttpResponseRedirect(reverse('vote:detail',
                                        args=(c.q.id,) ) )
