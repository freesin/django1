...
#모델클래스에 저장된 변수들에 맞춰 자동설정도 할수있고, 커스텀입력공간(비밀번호 확인)도 
#생성할수있음
...
#ex) 폼클래스객체.as_p() -> 해당 폼 객체에 저장된 입력공간들을 <input>태그로 변환하고,
...
#기존 개발 순서
#어플리케이션 생성 -> settings.py 등록 -> 모델 정의 -> 데이터베이스 반영 -> 뷰정의 ->
#템플릿 정의 -> URLConf 등록

#변경
#어플리케이션 생성 -> settings.py 등록 -> 모델 정의 -> 데이터베이스 반영 ->폼클래스정의
