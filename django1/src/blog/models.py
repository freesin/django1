...
class Post(models.Model):
    category = models.ForeignKey(PostType,
    ...
    #null(기본값 False)  : True값을 저장한 경우, 데이터베이스에 객체 저장 시
    #            해당 변수값이 비어있어도 생성되도록 허용
    #blank(기본값 False) : True값을 저장한 경우, 폼객체를 통한 사용자 입력공
    #     간(<input>) 제공 시, 해당 변수의 입력공간을 빈칸으로 허용
...
#ImageField를 사용할려면 Pillow 모듈이 설치되있어야 함
#Pillow 모듈 : 파이썬에서 이미지처리할 때 사용하는 대표적인 모듈
#pip install Pillow
class PostImage(models.Model):
    post = models.ForeignKey(Post,on_delete=models.CASCADE)
    #upload_to : 실제 파일이 저장되는 경로를 지정하는 매개변수
    #서버기준의 날짜데이터를 포함시킬 수 있음
    #%Y : 객체저장될때 서버기준의 년도
    #%m : 서버기준의 달
    #%d : 서버기준의 일
